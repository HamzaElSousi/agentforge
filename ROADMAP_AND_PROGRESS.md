...
- **V2 in progress:** Phase 1 (DAG via `depends_on`) ✅ and **Phase 2 (parallel execution on a thread pool)** ✅ — independent branches run concurrently with a thread-safe budget cap + cooperative cancellation. See [V2 design doc](docs/V2_DAG_PARALLELISM.md). Next: Phase 3 (fan-out), Phase 4 (parallel lanes in the UI).
...
| Parallel fan-out / fan-in | One agent spawns N workers over a list (`fan_out.over`); a join agent merges | 🔮 Phase 3 |
