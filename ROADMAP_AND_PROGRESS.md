...
- **V2 complete (Phases 1–4):** Phase 1 (DAG via `depends_on`) ✅, Phase 2 (parallel thread-pool execution) ✅, Phase 3 (dynamic fan-out: one agent → N worker instances → join) ✅, and **Phase 4 (parallel lanes in the web dashboard)** ✅ — concurrent branches render side-by-side, fan-out instances grouped; verified live (`docs/agentforge-fanout-lanes.gif`). See [V2 design doc](docs/V2_DAG_PARALLELISM.md).
